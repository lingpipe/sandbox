...
symbol \code{i} occurs in document \code{n}.  Here we have five
tokens, so \code{i} ranges between 0 (inclusive) and 4 (inclusive),
and we have 16 documents, so \code{n} ranges between 0 (inclusive) and
15 (inclusive).
...
It is this document-word matrix that is input to LDA.  First,
...
\code{Random} in \code{java.util}.  Here we provide the long integer
seed \code{43L}.  The behavior of \code{Random} is fully determined by
the seed (hence the qualifier \stringmention{pseudo-}).  By setting
...
constructor \code{Random()}, which combines the time (specifically
\code{System.nanoTime()}) and an internal key to generate a presumably
fresh seed.  

We sometimes use the following pattern to enable us a fresh random
seed each time but still reproduce outputs:
%
\codeblock{FragmentsLda.1}
...
handler's \code{fullReport()} method to print out information
