...
URL encoding, also called percent encoding, is used to encode both form data
and URIs which contain reserved or non-ascii characters.
The set of \emph{reserved characters} consists of those punctuation marks which
have a special meaning in a URL:
\mbox{\code{\! *  ' ( ) ; : @ \& = \$ , / ? \# {[} {]} }}
...
and then each byte of the UTF-8 representation is encoded separately.
The encoding for the byte value is a percent sign followed by the 
hex value of that byte.  The encoded byte will be in the range 00 to FF.
Thus the encoded byte can be expressed using the unreserved characters,
specifically the digits 0--9 and letters A--F.
Because the percent character is meaningful,
if the is occurs in the data, 
it too is URL encoded as \code{\%25}.
