...
the latter \code{String} contains exactly the \code{char separatorChar}
for convenience.
...
The name of the network storage device \code{{\bk\bk}ls-xl5de} whose
code {share} partition is mapped to drive \code{Z:\bk}
is not returned by the \code{File.listRoots()} method.
...
\code{FileFilter} which is passed in to the \code{listFiles} method.
\code{FileFilter} is an interface in \code{java.io} specifying a single
...
The example program \code{FilterFiles} takes two arguments:
the name of a directory and a file suffix. To find the total number of
files in the directory, it invokes \code{listFiles()} with no arguments.
%
\codeblock{FilterFiles.1}
% 
To find only those files in the directory which end with the specified suffix,
we define a class \code{SuffixFileFilter} which implements a \code{FileFilter}.
A \code{SuffixFileFilter} object contains member variable \code{String mSfx}.
The \code{accept} method returns \code{true} for filenames that 
end in \code{mSfx} and \code{false} otherwise.
%
\codeblock{FilterFiles.2}
%
To find only those files in the specified directory whose names 
end with the specified suffix we first instantiate a 
\code{SuffixFileFilter} and then pass that filter
into the \code{listFiles(FileFilter filter)} method.
%
\codeblock{FilterFiles.3}
%
We use the Ant target \code{filter-files} and pass in the
directory and suffix arguments as follows:
%
\commandlinefollow{ant -Ddir="/tmp" -Dsfx=".log" filter-files}
%
\begin{verbatim}
directory name=|/tmp|
12 files total
...
\subsection{System.out is a Print Stream}\label{section:system-out}

\code{System.out} is a \code{PrintStream} stream that corresponds to the Unix standard out (\code{stdout}) device.
The javadoc specifies that all characters printed by a \code{PrintStream} are converted
into bytes \emph{using the platform's default character encoding} (emphasis added).
This may be problemtic when using \code{stdout} for monitoring or debugging.%
%
\footnote{Debug by \code{printf} is surprisingly effective.}
%
In order to ensure that data sent to the console is correctly encoded we wrap \code{System.out} in an
\code{OutputStreamWriter} and explicitly specify the character encoding.  Then we create a new \code{PrintWriter}
that preserves the character encoding and has the same functionality as \code{System.out}.
%
\codeblock{FragmentsIo.16}
%
...
\subsection{URL Connections}\refsec{url-connections}
...
general web services.  This is covered in more detail in \refchap{web}.
