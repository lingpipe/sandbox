...
start "Download TREC 2006" wget -nc -w 5 -r -l 1 --accept "*.pdf" -P../data/citations/raw http://trec.nist.gov/pubs/trec15/papers/
...
start "Download ACL 2006" wget -nc -w 5 -r -l 1 --accept "*.pdf" -P../data/citations/raw http://acl.ldc.upenn.edu/P/P06/
...
start "Download NIPS 2006" wget -nc -w 5 -r -l 1 --accept "*.pdf" -P../data/citations/raw http://books.nips.cc/papers/files/nips19/
...
::
:: -l 1     Follow one link in the link graph.  (wget docs this as recursive)
::
:: --accept <pattern>  Only accept files matching pattern
::
:: -P<dir>  Write output to specified directory path
::
:: <url>    Where to start the downloda