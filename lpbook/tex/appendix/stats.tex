...
\subsection{Variance}\label{section:stats-binomial-variance}
...
distribution.  Suppose we have a discrete random variable $X$ taking
...
Entropy may also be expressed as expected log probability,
%
\begin{equation}
\entropy{X} = \expect{\log_2 p_X(X)}.
\end{equation}

\includegraphics[height=2.5in]{c:/lpb/tex/entropy.pdf}

Entropy has a natural interpretation in terms of compression.  If we
want to send the value of a message $X$ taking on values in $\nats$.
If the sender and receiver both know the distribution $p_X$ of
outcomes, the expected cost to send the message is $\entropy{X}$ bits.
