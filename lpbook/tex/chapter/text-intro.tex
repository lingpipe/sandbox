...
In this book we answer these questions by writing short, 
self-contained programs that carry out basic operations on text data
and report the state of the data before, during, and/or
after processing.

Understanding how character data is encoded into bytes and
how this data is sent to and retrieved from the operating system
is a prerequisite for building natural language processing applications.
Natural language processing (NLP) tasks over text data
include recognizing the language(s) used,
breaking the text into consituent sentences, phrases, words, and indexing
the text for search.
This book shows how to use existing open-source packages to
build robust, scalable applications for natural language text data.
...
UTF-8 character encoding (which we will explain in detail in \refchap{char}).
...
%
\urldisplay{http://tomcat.apache.org}.
%
