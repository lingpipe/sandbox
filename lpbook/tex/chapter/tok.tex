...
as a static constant in the class.  For consistency, these singleton
constants all have the variable name \code{INSTANCE}.
...
\code{TokenizerFactory} interface, most classes extending
\code{Tokenizer} are not delcared to be public.  Instead, only the
factory is visible, and the documentation for a tokenizer's behavior
will be in the factory's class documentation.
...
base class to throw an \code{UnsupportedOperationException}.  All of
LingPipe's built-in tokenizer factories produce tokenizers that
properly handle token positions.  

User-defined ubclasses that need token positions should override these
methods.  Tokenizer filters should almost always just pass the
positions of the tokens being modified, so that the derived token may
be linked back to its source in the original text.
...
\subsection{The \code{IndoEuropeanTokenizerFactory} Class}\label{section:tok-indo-euro}
...
or above \unicode{10000}, will produce two tokens (see the section on UTF-16
...
mode (see the section in the companion volume {\it Text Processing in Java}
for more on the find operation).  An instance may be constructed from a
\code{Pattern}, or from a \code{String} representing a regular
expression with optional integer flags (see the section on modes in patterns in the regular expression chapter of the companion volume, {\it Text Processing in Java}, for
...
the section on line-ending in the regular expression chapter of the companion
...
the section on serialization proxies in the I/O chapter of the
companion volume, {\it Text Processing in Java}).
...
the following code for a variable \code{f} of type \code{TokenizerFactory},
%
\codeblock{FragmentsTok.8}
%
...
In linguistics, morphology is the study of how words are formed.
Morphology crosses the study of several other phenomena, including
syntax (parts of speech and how words combine), phonology (how words
are pronounced), orthography (how a language is written, including how
words are spelled), and semantics (what a word means).
...
\code{coding} are all inflectional variants of the same underlying
word, differing in person, number, tense, and aspect.  We get
agreement between verbs and subject nouns in English for these
features.  For instance, a person might say \stringmention{I code} in
referring to themselves, but \stringmention{she codes} when referring
to their friend writing code.  Other languages, such as French or
Russian, also have agreement for gender, which can be natural (male
person versus female person versus versus non-person) or grammatical
(as in French).  Natural gender is based on what is being talked
about.  That's the distinction we get in English between the relative
pronouns \stringmention{who}, which is used for people and other
animate objects, and \stringmention{what}, which is used for inanimate
objects.  For instance, we say \stringmention{who did you see?} if we
expect the answer to be a person, and \stringmention{what did you
  see?} if we want a thing.
...
a sequence of consonants and an inflected form mixes in vowels.
...
so that the token \stringmention{brave} starts at position
...
(ICU) package for normalizing sequences of Unicode characters in the
companion volume, {\it Text Processing in Java}.  In this section,
we'll show how to plug that together with tokenization to provide
tokenizers that produce output with normalized text.
...
We wrote a demo class, \code{UnicodeNormTokenizerFactory}, which uses
the the \code{Transliterate} class from ICU to do the work (see the
section on transliteration in the companion volume, {\it Text
  Processing in Java}, for an introduction to the class and more
examples).  A transliterator is configured with a transliteration
...
transliteration scheme (see the section on Unicode transliteration
with ICU in the companion volume, {\it Text Processing in Java}, for
a description of the available schemes and the language for
composing them) and property \code{text} for the text to modify.
%
\commandlinefollow{ant "-Dtext=Déjà vu" "-DtranslitScheme=NFD; [:Nonspacing Mark:] Remove; NFC" unicode-norm-tokens}
\begin{verbatim}
...
to use UTF-8 (see the section on standard input and output in the
...
it's an opening quote and two apostrophes (\charmention{''}) if it's a
...
operations (see the section on replace in the regular expression chapter
of the companion volume, {\it Text Processing in Java}),
%
...
instance of \unicode{0022}, \unicodedesc{quotation mark},
(\charmention{"}), the ordinary typewriter non-directional quotation
mark symbol, with \unicode{201C}, \unicodedesc{left double quotation
  mark} (\charmention{``{}}).  The second replacement replaces a
quotation mark after characters of class \code{Z} (separators such as
spaces, line or paragraph separators) or class \code{Ps}, the
start-punctuation class, which includes all the open brackets
specified in the original SED script.  The context characters are
specified as a non-capturing lookbehind (see the relevant section of
the companion volume). The third replacement converts remaining quote
characters \unicode{201D}, \unicodedesc{right double quotation mark}
...
\section{Adapting to and From Lucene Analyzers}\label{section:tok-adapting-lucene-analyzers}

In the section Lucene analysis in the Lucene chapter of the companion
volume, {\it Text Processing in Java},  we discuss the role of Lucene's
analyzers in the indexing process.  A Lucene document maps field names
to text values.  A Lucene analyzer maps a field name and text to a
stream of tokens.  In this section, we show how to convert a Lucene
analyzer to a LingPipe tokenizer factory and vice-versa.
...
\begin{quote}
\begin{RLtext}
أولويوكي $\mbox{\rm (Oulujoki)}$ نهر في مقاطعة أولو بفنلندا. 
منشؤه هو بحيرة أولويارفي، يغطي جزءاً كبيرا من 
إقليم كاينو ويصب في بحر البلطيق. النهر مجهز ب 12 
محطة لتوليد الطاقة الكهرمائية بقدرة إنتاجية 
تبلغ 541 ميغاواط. ...
\end{RLtext}
\end{quote}
%
...
file=config\arabic-sample.utf8.txt
 -----text-----
!RL[أولويوكي] (Oulujoki) ...
...
from: http://ar.wikipedia.org/wiki/!RL[أولويوكي]_!RL[نهر]
-----end text-----
START   END TOKEN
    0     8  |!RL[اولويوك]|
   10    18  |oulujoki|      
...
sets up the properties for the next token if there is one.
%
...
\subsection{The \code{Tokenization} Class}\label{section:tokenization-tokenization-class}
