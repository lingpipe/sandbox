...
that is indexed.  The values \code{Index.ANALYZED\_NO\_NORMS} and
\code{Index.NOT\_ANALYZED\_NO\_NORMS} disable storage of these
...
been removed.  The stop words \stringmention{will}, \stringmention{for}
and \stringmention{the} are also removed from the output.  Unlike
...
\stringmention{smith} and \stringmention{pay} is 2, because the stopword
\stringmention{will} was removed between them.  The start (inclusive)
...
subdirectory \code{data/federalist-papers} which contains the Project Gutenberg
distribution of the plain-text version of the{\it Federalist Papers} along with the
shell script \code{get\_papers.sh}.  Running this script will create a directory called \stringmention{texts}
and populate it with the individual papers, one paper per file.  
Once unpacked, we use this directory as the \code{doc.dir} argument.
...
\reffig{luke-overview} illustrates how Luke presents an overview
...
In earlier versions of Lucene, document deletion was handled by
an \code{IndexReader}.%
%
...
The code is in the main() method of the demo class \code{LuceneDelete}.
...
in to the \code{IndexWriter} constructor.
%
