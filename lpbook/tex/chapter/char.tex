...
is implemented by \code{StringBuilder}, a mutable implementation


The two most commonly used are \code{String} and \code{StringBuilder},
which provide an immutable sequence of characters and mutable sequence
respectively.

 class
\code{String} provides an immutable implementation and
\code{StringBuilder} a mutable implementation.  The older
\code{StringBuffer} class provides a synchronized version of
\code{StringBuilder}, but is rarely needed.

%
%
\footnote{The class \code{StringBuilder} is an unsynchronized version
of \code{StringBuffer}.  Because it is rarely desirable to write to
