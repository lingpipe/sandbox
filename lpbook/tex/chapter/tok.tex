...
character as a token.  As of LingPipe 4.0, this is each \code{char}
value, so unicode code points outside of the basic multilingual plane
(BMP) will produce two tokens (see \refsec{utf-16}).%
%
\footnote{In the future, this default behavior may be extended to
allow the class to be configured so that it treats characters outside
of the BMP as single tokens.}
%
The class is thread safe and serializable.  There is no constructor,
only is a static constant \code{INSTANCE} that is also the result of
deserialization.
