...
%
\begin{verbatim}
( 4, 8):LOC  0.89     (41,45):PER  0.21
( 0, 5):PER  0.46     (41,45):LOC  0.09
(41,45):ORG  0.43     ( 0,18):PER  0.02
\end{verbatim}
%
...
\code{main()} method, which has no command-line arguments.  It
starts by constructing an evaluation and populating it.
%
...
\end{tabular}
\end{center}
\caption{Example of scored precision-recall evaluation.  The responses
  are shown with their correctness, sorted by score.  There is a
  single missed positive item for which there was no response, so we
  include it at the bottom of the table without a score.  In total,
...
The next four columns represent the counts in the various cells of a
precision-recall matrix, TP, TN, FP, and FN.  These are the basis for
the computation of the next three columns, which are recall,
precision, and specificity (at rank $N$).  These values are available
through the evaluation API, as are many other statistics.  For recall,
this is the fraction of all reference positive items that have been
retrieved at the specified rank or before.  The highest recall
achieved is only 0.80 because we added a missed item, for a total of
five reference positive items.  Every time we hit a correct answer,
the recall (sensitivity) and precision go up and specificity stays the
same.
...
%
The output from this block of code for the current example is as follows.
%
\begin{verbatim}
...
curve is a plot of sensitivity versus $1 - \mbox{specificity}$.  

Although we have a method \code{addMisses(int)} to add overall false
positive counts, we do not have a similar arrangement for ROC curves
to add overall false negative counts, as we'd require to compute
proper ROC curves in the face of partial system responses.  
Thus use of ROC curves should be restricted to situations in which
all possible categories have responses.  
