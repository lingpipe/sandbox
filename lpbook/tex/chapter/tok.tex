...
The negative lookahead for the basic tokenizer makes sure that periods
are not picked up as part of tokens if they are followed by the end of
input or a non-emptysequence of punctuation characters including final
punctuation like right brackets (Unicode class \code{Pf}), the double
quote character or the apostrophe.  For example, 
%
...
negative lookahead disjunct appearing before the general disjunct,
the period would've been included.

The same negative lookahead makes sure that the contraction
\stringmention{n't}, which has an internal apostrophe, is not
included in the previous token, as we saw a few examples back.
...
\footnote{Although the double quote character itself can be escaped in
  most shells, Ant's scripts can't handle \code{-D} property
...
There are several problems with the Penn Treebank tokenizer as presented.
The problems are typical of systems developed on limited sets of data with
the goal of parsing the data at hand rather than generalizing.  

We fixed the problem of only recognizing ASCII quotes and only
recognizing ASCII brackets by generalizing to Unicode classes in our
regular expressions.

A remaining problem is that we don't handle sentence-final, but
non-string-final periods.  For instance, if we run
%
\commandlinefollow{ant -Dtext="John says, 'hello.'" treebank-tokens}
%
we'll find the token \charmention{hello.} (including the period
character).  The problem is with the rule that splits out final
periods in both our code and the SED script from which it was derived.
We'll tackle this issue again when it comes to sentence detection
(see \refchap{sentence}).
