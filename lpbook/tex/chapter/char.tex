...
and hexadecimal.  As may be gathered from its name,
...
Each Java installation has a default character encoding.  Each Java
installation also supports a range of other encodings.  Every
compliant Java installation supports all of the UTF encodings of
Unicode.  Other encodings may be added at the installation level,
or implemented programatically within a JVM instance.
...
Because Java programs can be written using a range of character
encodings, these quoted character literals may refer to just about any
character.  In general, inside a Java program, you can refer to a
character using a Unicode escape, using \code{{\bk}u}\varcode{xxxx}
for the Unicode character with hexadecimal code point \varcode{xxxx}.
For instance, these can be used inside of character literals, so
...
The use of Unicode escapes like \code{{\bk}u0061} makes it possible
to write programs using non-ASCII characters using only ASCII.
