...
models.  The outcome of a classification is one of finitely many
categories, not a continuous scalar value like a floating-point number.
...
integers, which are also called ``natural numbers''.  Examples of
count variables include the number of times a word appears in a
document, the length of a document's title in characters, and the
number of home runs a baseball player gets in a season.  Classifiers
like naive Bayes convert word count data into categorical outcomes
based on a probability model for documents (see
\refchap{naive-bayes}).
...
standard document search problem (see the chapter on the Lucene search
library in the companion volume, {\it Text Processing in Java}) may be recast
...
\section{Kinds of Classifiers}

LingPipe provides several varieties of classifiers.  These are
ordered in terms of the amount of information they provide as
the result of classification.  In this section, we describe them
in order from least informative output to most informative.

The reason there are multiple classification classes is that
different classification techniques have different capabilities.

Each type of classifier comes with its own evaluation class
which is tailored to the information provided by that kind of
classifier.


\subsection{First-Best Classifiers and Classifications}
...
A first-best classifier will implement the interface \code{BaseClassifier<E>},
providing classifications of type \code{E} objects.  The interface defines
a single method, \code{classify(E)}, which returns a first-best
classification for the specified input.  

The return type of the classify method is \code{Classification}.  The
\code{Classification} base class implements a single method
\code{bestCategory()}, which returns a string representing the
category.  While this could've been done with a return type consisting
of just a string, the base class \code{Classification} is going to be
extended to richer types of classification, which would not be possible
with \code{String}.

A \code{Classification} instance is constructing from the string
corresponding to its best category, using constructor
\code{Classification(String)}.

Evaluation of first-best classifiers is carried out using the class
\code{BaseClassifierEvaluator}.  Base evaluators track confusion
matrices, which represent the full empirical response profile of a
first-best classifier by category.  From a confusion matrix, it's
possible to derive all sorts of other evaluations, which are described
in the Javadoc for the evaluator and exemplified later in this chapter.


\subsection{Ranking Classifiers and Classifications}
...
A ranking classifier implements the interface \code{RankedClassifier}
which extends \code{BaseClassifier}.  In addition to returning the
best category, a ranked classifier is able to return additional
guesses along with their ranks.  The idea is that the higher the rank,
the more confident the classifier.  Because there is more than one
result, a ranked classifier implements a method \code{size()}
returning an integer indicating the number of categories in the
classification.  The method \code{category(int)} returns the string
corresponding to the category at a specific rank.  Assuming the size
is non-zero, the method call \code{category(0)} will return the same
value as \code{bestCategory()}.  If there are at least two categories,
the call \code{category(1)} returns the second-highest ranking
classification by the classifier, and so on.

A ranked classifier is constructed from a sequence of categories
ordered from best guess to worst guess using the constructor
\code{RankedClassification(String[])}.  

A ranked classifier is evaluated using the \code{RankedClassifierEvaluator}.
The ranked evaluator extends the base evaluator the same way that
a ranked classifier extends a base classifier and a ranked classification
extends a first-best classification.  A ranked classifier evaluator
adds ranked evaluation statistics like the mean reciprocal rank of the
true answer.


\subsection{Scored Classifiers and Classifications}
...
A scored classifier implements the interface \code{ScoredClassifier},
which extends \code{RankedClassifier}.  In addition to returning sizes
and categories at ranks, the method \code{score(int)} returns a
double-precision floating-point score for the result at the specified
rank.  These scores should be decreasing as the rank decreases.  
That is, \code{score(0)} should be less than or equal to \code{score(1)}
and so on.

A \code{ScoredClassification} is constructed from parallel arrays of
of categories and scores using the constructor
\code{ScoredClassification(String[],double[])}.  The scores must be
sorted in decreasing order; if they are not, the constructor throws an
illegal argument exception.  There is a utility method
\code{create(List<ScoredObject<String>>)} that creates a scored
classification from an unsored list of scored strings.  The
\code{ScoredObject} class, in LingPipe's package \code{com.aliasi.util}
is such that an instance of \code{ScoredObject<String>} implements
a method \code{getObject()} returning a string and a method \code{score()}
returning a double-precision floating-point score.

A scored classifier is evaluated with a \code{ScoredClassifierEvaluator},
which extends the ranked evaluator class.  The results of a scored
classifier evaluation rare stored in a \code{ScoredPrecisionRecallEvaluation}
object.  This evaluation class supports results like precision-recall and
ROC curves, recall at specific ranks, and other evaluations depending on
scores.

 
\subsection{Conditional Classifiers and Classifications}\label{section:conditional-classifier}
...
A conditional classifier implements the interface
\code{ConditionalClassifier}, which extends \code{ScoredClassifier}.  
It adds a method \code{conditionalProbability(int)} for returning
the conditional probability of the result at the specified rank.
It also contains a method \code{conditionalProbability(String)} which
returns the probability estimate for a specified category.

There are several constructors for conditional classifiers.  The
basic constructor is \code{ConditionalClassifier(String[],double[])},
which takes a pair of parallel arrays representing the categories
and their conditional probabilities, in order.  The sum of the
probabilities must be 1.  An optional third argument may be provided
to specify the tolerance of the probabilities summing to 1.  
A second constructor is \code{Conditional(String[],double[],double[])},
with three parallel arrays, the first floating point one of which
contains scores and the second of which contains conditional probabilities.
The scores do not need to be the same as the conditional probabilities,
but both must be presented in descending order.  If the scores or
probabilities are not in descending order, an illegal argument exception
is raised.  An exception is also raised if the arrays do not ahve the
same length.

There are two convenience static factory method for creating
conditional classifications.  The first,
\code{createLogProbs(String[],double[])} takes probabilities on the
log scale and normalizes them to proper probabilities by
exponentiating and sorts them appropriately.%
%
\footnote{The normalization is the usual softmax operation.  With
$K$ categories with log probability scaled scores $x_k$, we set
$\mbox{Pr}(\mbox{cat}=k) \propto \exp(x_k)$, so that 
\begin{equation*}
\mbox{Pr}(\mbox{cat}=k) = \frac{\exp(x_k)}{\sum_{k'=1}^K \exp(x_{k'})}.
\end{equation*}}
%
The second static factory method has the same signature with a
different name, \code{createProbs(String[],double[])}.  This takes
unnormalized probabilities on the linear scale, which must be
non-negative, normalizes them to sum to 1, sorts them, then returns
the corresponding conditional classifier.%
%
\footnote{The normalization is the usual rescaling.  With $K$
categories with linear probability scaled scores $y_k$, we set
$\mbox{pr}(\mbox{cat}=k) \propto y_k$, so that
\begin{equation*}
\mbox{Pr}(\mbox{cat}=k) = \frac{y_k}{\sum_{k'=1}^K y_{k'}}.
\end{equation*}}

A conditional classifier is evaluated using a
\code{ConditionalClassifierEvaluator}, which extends the scored
classifier evaluator class.  A conditional classifier provides
average conditional probabilities as well as conditional one-versus-all
evaluations.  


\subsection{Joint Classifiers and Classifications}
...
A joint classifier implements the interface \code{JointClassifier},
which extends \code{ConditionalClassifier}.  The reason it can extend
the conditional classifier class is that given joint probabilities,
it's possible to calculate conditional probabilities, but not
vice-versa.  The joint classifier interface defines a classify method
that returns a \code{JointClassification}, which extends the
\code{ConditionalClassification} class.

The basic constructor is \code{JointClassification(String[],double[])},
where the first array is the array of categories and the second
the array of log (base 2) joint probabilities.  The arrays must be
the same length and the probabilities must be in descending order.
A second constructor, \code{JointClassification(String[],double[],double[])},
takes separate score and joint probability arrays in the same way
as we saw for conditional classifiers.  The joint probabilities are
used to define conditional probabilities in the usual way.%
%
\footnote{If we have $K$ categories with log (base 2) joint
  probabilities and $w$ is the input, let $y_k = \log_2 \mbox{Pr}(\mbox{cat}=k,\mbox{in}=w)$ be the
  log (base 2) joint probability of category $k$ and input $w$.  
  where $c_k$ is a category and $w_k$ is the object being classified,
  $\mbox{Pr}(\mbox{cat}=k|\mbox{in}=w) \propto 2^{y_k}$ and we normalize
  as with any probabilities that are known up to a proportion, by 
%
\begin{equation*}
\mbox{Pr}(\mbox{cat}=k|\mbox{in}=w) = \frac{2^{y_k}}{\sum_{k'=1}^K 2^{y_{k'}}}.
\end{equation*}}

A joint classifier is evaluated using an instance of
\code{JointClassifierEvaluator}, which extends the conditional classifier
evaluator.  Additional functionality includes joint probability
evaluations.  These are the traditional evaluation measures for
joint statistical models and are based on the probabilty assigned to
the training or test instances by the model.  
...
retrieval system like Lucene (see the chapter on Lucene in the companion
volume, {\it Text Processing in Java}), because of all
...
%
In practice, we don't know the values of $I$ or $J$, but we will know
$M$ and $N$.  We are left with a pair of equations,
%
\begin{align}
...
\end{align}
%
Solving for $I$ and $J$ in terms of $M, N, \theta_0$, and $\theta_1$
yields
%
\begin{align}
J &= \frac{\theta_1 N + \theta_1 M - M}
         {\theta_1 + \theta_2 - 1}, \mbox{ and}
\\[8pt]
I &= M+N-J.
\end{align}
%
Thus This allows us to estimate the prevalence of positive
