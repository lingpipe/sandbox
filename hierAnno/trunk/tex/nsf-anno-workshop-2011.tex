...
\vspace*{12pt}
\noindent
\spc{\huge A Hierarchical Model}
\\[4pt]
\spc{\huge  of Corpus Annotation}
\\[24pt]
\spc{\large Bob Carpenter,} 
\\[12pt]
\spc{\itshape Columbia Uni. \ \& \ LingPipe, Inc.}
\\[24pt]
\spc {\small NSF Workshop on Collaborative Annotation}


\mypart{$\kappa$ Statistics}
...
\begin{itemize}
\footnotesize
\item Correctness of corpus
\item Accuracy/bias of each annotator (adjust ``votes'')
\item Difficulty of tagging task (replace $\kappa$)
\item Prevalence of item types in corpus
\item Mean annotator accuracy and bias
\item Annotator variability
\item Predict effect of new annotation 
\\
\item Bayesian propagation of uncertainty in inference
\end{itemize}


\sld{A (Mostly) Generative Model Sketch}

\begin{itemize}
\item $\pi \ |\ $ \ \hfill item distribution
...
\sld{A Fully Generative Model Sketch}

\vspace*{-4pt}
\begin{itemize}
\item $\alpha \ | \ $  \hfill distribution of item distributions
\item $\pi \ | \ \alpha$ \hfill item distribution
\item $z_i \  | \ \pi$ \hfill true category for item $i$
\\[-6pt]
\item $\beta \ | \ $ \hfill distribution of annotator responses
\item $\theta_j \ | \ \beta$ \hfill response for annotator $j$ 
\\[-6pt]
\item $y_{i,j} \ | \ z_{i},\ \theta_{j}$ \hfill label by annotator $j$ for item $i$
\end{itemize}
\vfill
\spc{\small (Carpenter 2008)}


\sld{Estimation}

\begin{itemize}
\item Point Estimates (MLE or MAP)
\begin{itemize}
\footnotesize
