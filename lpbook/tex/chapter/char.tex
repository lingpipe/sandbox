...
keyword.  Methods may also be marked with \code{synchornized}, which
...
There's no getting around it -- Java objects are heavy.  The language
specification does not determine how objects are represented on disk,
and different JVMs may handle things differently.  We'll discuss how
the 32-bit and 64-bit Sun/Oracle JVMs work.

First, every object contains two pieces of information.  The first 
establishes the object's identity, and is also used for locking
and the \code{hashCode()} returned by \code{Object}.  Even if
an object is relocated in memory, its reference identity and locking
properties remain fixed.  The second piece of information is 
a reference to the class definition.  

A reference-sized chunk of memory is used for each piece of
information, so that's four bytes on a 32-bit architecture and eight
bytes on a 64-bit architecture.  Thus an instance of \code{Object} is
8 bytes in a 32-bit JVM and 16 bytes in a 64-bit JVM.
...
and 8 bytes for \code{double}}.  To help with word alignment, they are
ordered in decreasing order of size, with doubles and longs coming
first, then ints and floats, down to bytes.
...
Subclasses need to use their superclass memory layout for efficiency,
but this can lead to badly aligned memory for member variables of the
subclass.  For instance, a superclass with a single \code{int} member
variable must be padded to a round multiple of either 4 or 8 bytes,
depending on the architecture.  To save space, subclasses can pack
variables that fit into the padding in some cases. 

A serious drawback to Java for high-performance computing is that
there is no equivalent to C's \code{struct} types, which have no
runtime memory overhead and may additionally have types unfolded into
them.  To get around this problem, LingPipe employs parallel arrays
rather than defining object wrappers.
...
range raises an \code{IndexOutOfBoundsException}.

In addition to the usual object header (8 bytes in 32-bit and
16 bytes in 64-bit Sun/Oracle JVMs), they also store an integer
length in 4 bytes.  After that, they store the elements one after
the other.  If the values are \code{double} or \code{long} values,
there is an additional 4 bytes of padding after the length so
that they start on 8-byte boundaries.
...
Strings may also be constructed by copying the current contents of a
string builder or string buffer (see the next section for more about
these classes, which implement the builder pattern for strings).
...
contstants including literals, will be shared.  Thus 
\code{"abc" == "abc"} will evaluate to true because the two literals
will denote reference identical strings.  This is carried out as if
\code{intern()} had been called on constructed strings (see below).
...
%
For instance, \stringCode{abc} is less than \stringCode{abde}
\stringCode{ae}, and \stringCode{e}.  If we finish either string, we
return the difference in lengths.  Thus \stringCode{a} is less than
\stringCode{ab}.  If we finish one of the strings and the strings are
...
\subsection{Interning Canonical Representations}
...
If two strings are equal, they will \code{intern()} to reference
identical objects.  For instance,  suppose we call
%
\begin{verbatim}
...
Thus two calls to \code{intern()} on two reference distinct strings
that are equal (in the sense of \code{equals(Object)})
...
to any other equal string that has been internalized.

deeply tied into the implementation of strings in the JVM.  
...
\code{replace}\varcode{X}\code{()} methods based on regular expressions.
...
It is common to create string objects using the builder pattern, which
is implemented by \code{StringBuilder}.  Conveniently, the string
builder class implements the \code{CharSequence} interface.  In cases
where character sequence arguments suffice, this allows us to bypass
the construction of a string altogether.

%
\footnote{The class \code{StringBuffer} is a synchornized version
of \code{StringBuilder}.  Because it is rarely desirable to write to
a string buffer concurrently, \code{StringBuilder}.}

Java's \code{String} class is essentially an immutable sequence of
characters.  The \code{String} class implements the \code{CharSequence}
interface, which is essentially a sequence of characters.

Various operations are built into the language to support
string processing.  
