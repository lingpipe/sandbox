...
\end{itemize}
\item Use proportion of 1 labels for prevalence $\pi$
...
\item Use agreement on negatives for specificity $\theta_{0}$
\item Use agreement on positives for sensitivity $\theta_{1}$
...
\end{array}$
\\[8pt]
\hspace*{-8pt}$\mbox{\rm Pr}(c_i=1|\theta,x) = \frac{\displaystyle 0.00975}{\displaystyle 0.00975+0.0768} = 0.1126516$
\end{itemize}
...
\item Works for any marginal or joint estimate
\end{itemize}


\sld{Gibbs Samples For Inference}
\begin{itemize}
\item Samples $\phi^{(n)}$ support plug-in inference (like the bootstrap)
\begin{itemize}
...
\item Cross-hairs at simulated values
\\[4pt]
\begin{tabular}{ll}
...
\item Estimates match sample averages; more variance on sensitivity 
\end{itemize}
...
\begin{itemize}
\item Used published gold standard as gold standard
...
\begin{itemize}
\item Good annotators but hard items
\item Mediocre annotators with average difficulty items
\item Bad annotators but easy items
\end{itemize}
\end{itemize}
...
\item Other alternatives for discriminativeness and not
\vfill
...
\item Do items all have true categories
\begin{itemize}
\item Vague coding standard
\item Marginal items
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item May indicate problem with coding standard
\end{itemize}
\item Alternative to explicitly modeling difficulty
\end{itemize}

\sld{Speed versus Accuracy}
\begin{itemize}
\item Assume all we want accurate gold standard
...
\begin{itemize}
\item New label for uncertainly labeled item
\item Label for currently unlabeled item
...
\begin{itemize}
\item Replacement for $\kappa$ with predictive power
\item Sensitivity and Specificity priors $(\alpha,\beta)$ model
\begin{itemize}
\item Average annotator accuracy
...
\item Demonstrated theoretical effectiveness 
...
\begin{itemize}
\item Given estimates of sensitivity, specificity and prevalence, may
\begin{itemize}
...
\end{itemize}
\item May estimate confidence intervals for $\kappa$ w/o model
\end{itemize}
