...
\firstchar{J}ava uses Unicode character representations.
...
\subsubsection{What is a Character?}
...
\section{Coded Sets and Encoding Schemes}
...
\section{Legacy Character Encodings}

We consider in this section some widely used character encodings, but
because there are literally hundreds of them in use, we can't be exhaustive.  
The characters in these and many other character sets are part of Unicode.
In Java the only thing you need to know about a character encoding
in order to convert data in that encoding to Unicode is its name.


\subsection{ASCII}\label{section:ascii}
...
keys of an American typewriter.

The ASCII codes 0-31 are the
control characters.  These are not characters, per se, but rather
control signals sent to attached devices such as printers and tape drives.
The control characters include line feed, backspace, ring margin bell,  
and data transmission controls.
...
\subsection{Latin1 and the Other Latins}\label{section:latin1}
...
\section{Unicode}\label{section:unicode}

Unicode is the de facto standard for character sets of all kinds.  
It contains over 1,000,000 distinct characters drawn from hundreds of languages.
Technically, it assigns characters to code points 
in the range 0x0 to 0x10FFFF (0 to 1,114,111 in decimal notation).
...
Unicode's first 256 code points (0--255) exactly match
...
This is because the first 32 ASCII codes (0x00 through 0x1F) 
...
Of the ASCII control characters only tab (0x09), carriage return (0x0A), 
and linefeed (0x0D) are valid Unicode characters.
...
point 65 (0x41), which represents capital A in ASCII, Latin1 and
Unicode, is conventionally written \unicode{0041} for Unicode.

\subsection{Unicode Planes: the Basic Multilingual Plane and Supplementary Planes}\label{section:unicode-planes}

All unicode code points are in the range 0x0 to 0x10FFFF.
These are logically divided into 17 \stringmention{planes}
...
For example, if we write capital A as \unicode{000041}, we 
...
Plane 0 is known as the \stringmention{Basic Multilingual Plane} (BMP).
...
Planes 1 through 17 are known at \stringmention{Supplementary Planes}.
Plane 1 is known at the \stringmention{Supplementary Multilingual Plane} (SMP).
...
Plane 2 is known as the \stringmention{Supllementary Idiographic Plane} (SIP).
...
\subsection{UTF: Unicode Transformation Formats}

Unicode assigns characters to code points.  The Unicode Transformation
Formats map these code points to a sequence of bytes.
...
like Spanish or English, which have code points below 256 and therefore
can fit into a single byte.  They are also problematic in that a 3-byte
sequence takes up 1.5 words on 32-bit machines but less than a word
on 64-bit machines.

There are three standard encodings for Unicode code points that are
specified in the Unicode standard, UTF-8, UTF-16, and UTF-32.%
%
...
\subsection{UTF-16}\label{section:utf-16}
...
In UTF-16, code points below \unicode{10000}, i.e. 
code points in the Basic Multilingual Plane (BMP, see \refsec{unicode-planes}),
...
Code points from the Supplementary Planes
are represented using four bytes arranged in two pairs called \stringmention{Surrogate Pairs}.
...
The binary representation of code point 0x10FFFF is 
\mbox{\code{100001111111111111111}}, which is 21 bits long.
...
Calculating the two pairs of bytes
...
%
Happily, The Java \code{Character} class provides static methods
that do this bit twiddling for us.
The static method \code{isSupplementaryCodePoint(int)} 
can be used to identify supplemental code points. 
The static methods \code{highSurrogate(int)} and 
\code{lowSurrogate(int)} return the specified byte pairs.


\subsection{UTF-8}\label{section:utf-8}
...
\subsection{Non-Overlap Principle}\label{section:utf-nonoverlap}
...
If we look at the total number of bits that contain code point information,
we see that for a singleton byte this is 7 because 
the first bit is 0, identitfying the sequence type,
and the remaining 7 bits contain code point information.
For a two-byte UTF-8 sequence, the first byte contains 5 bits of
code point information and the second byte contains 6 bits since
the first two bits must be \code{10}, for a total of 11 bits worth of information.
For a three-byte UTF-8 sequence, the first byte contains 4 bits, and the
next two contain 6 bits apiece, for a total of 16 bits of information.
For a four-byte UTF-8 sequence, the first byte contains 3 bits and the
next three contain 6 bits, for a total of 21 bits of information.%
%
\footnote{In theory, UTF-8 can represent twice as many code points as UTF-16.
In practice, the Unicode standard limits the maximum code point to 0x10FFFF.}
%

The non-overlap principle means that it is possible to reconstruct
...
\subsection{Byte Order Marks}\label{section:byte-order-marks}
...
\subsection{Noncharacters}\label{section:noncharacters}

Unicode specifies 66 codes points as non-characters, including
the last two code points in each plane, U+FFFE, U+FFFF, U+1FFFE, U+1FFFF
and so forth.
Note that the code point U+FFFE has the
reverse binary sequence of the byte order mark (U+FEFF). 
If this is found in some text, it is an indication that
it was processed using the wrong byte-order.

Software implementations are therefore free to use these code points for internal use.
In Java, the interface \code{java.text.CharacterIterator} defines the constant \code{DONE}
%
\codeblock{FragmentsChars.21}
%
Because this value should not occur in any valid Unicode 
string, it is used as the value returned when the iterator 
has reached either the end or the beginning of the text. 

\subsection{Character Types and Categories}\label{section:unicode-categories}
...
\subsection{Normalization Forms}\label{section:unicode-normalization-forms}
...
\subsection{Encoding Problems and Considerations}\label{section:char-probs}
...
Java's primitive \code{char} data type stores Unicode characters in UTF-16.
A \code{char} is 16-bits.
For Unicode code points below \unicode{FFFF},  
a \code{char} transparently contains the value of that code point.
Unicode code points above \unicode{FFFF} are encoded as a 
sequence of two \code{char}s using the UTF-16 encoding, (see \refsec{utf-16}).

A \code{char} is essentially a 16-bit (two
...
%
The expression \code{0x00E0} is an integer literal in hexadecimal
...
\section{\code{Character} Class}\label{section:character-class}
...
supplemental code points, (see \refsec{unicode-planes} and \refsec{utf-16}).
In Java, code points about 0xFFFF are encoded as a pair of \code{chars}.
Givan an array of \code{char}, there is not necessarily a one-to-one
correspondance between the encoded characters in the text
and the \code{char} elements of the array.
The method \code{codePointCount(char[],int,int)} returns the number of 
code points encoded by the specified \code{char} array slice.
The method \code{codePointAt(char[],int)} determines the code point
that starts at the specified index int he array of \code{char}.  

The static method \code{charCount(int)} returns the number of \code{char} values
required to represent the code point (so the value will be 1 or 2,
reflecting 2 or 4 bytes in UTF-16).  
The static method \code{Character.toChars(int)} does the conversion.
The methods
...
For a code point with \code{int} value \code{x}, if
\begin{verbatim} 
...
\subsection{Unicode Character Categories and Properties}\label{section:char-unicode-cats}

Every Unicode character is assigned a general category and subcategory.
The general categories are: letter, mark, number, punctuation, symbol, 
or control (for formatting or non-graphical character).
...
Unicode also assigns character properties to each code point.
The are used by processes such as finding line breaks,
laying out script direction, or applying controls.
...
Because a code point may require a pair of \code{char} instances,
...
Because lengths are instances of \code{int}, they are only allowed to be as
...
\subsubsection{From \code{char} Values}\label{section:string-construct-chars}
...
\code{char} values is a legal UTF-16 sequence.  Thus we don't know if
...
\subsection{Testing String Validity}\label{section:valid-utf16}

As observed in section \refsec{string-construct-chars}, it is possible
...
\code{ValidateUtf16} to do this validation:
%
\displ{\filepath{src/chars/src/com/lingpipe/book/chars/ValidateUtf16.java}}
%
\codeblock{ValidateUtf16.1}
%
The static utility method \code{isValidUTF16(CharSequence)} 
checks that every
high surrogate UTF-16 \code{char} value is followed by a low surrogate
UTF-16 value and that every low surrogate is preceded by a high surrogate.
Surrogacy is defined as by Java's
\code{Character.isHighSurrogate()} and
\code{Character.isLowSurrogate()} methods (see \refsec{char-character-utils}).
It uses the method \code{Character.isValidCodePoint(int)} to check
the code point values.

We test this method on a String constructed from
a pair of \code{char}s which correspond to
a Unicode code point in the supplemental plane.
%
\codeblock{ValidateUtf16.2}
%
We expect this test to succeed.
Then we swap the order of the chars and run this test again.
%
\codeblock{ValidateUtf16.3}
%
We expect this test to fail.
We run this code using the Ant target \code{validate-utf16}
and see that this is indeed what happens.
%
\commandline{ant validate-utf16}%
\begin{verbatim}
UTF-16 chars in s: [ d841 df0e ], s is valid string: true
UTF-16 chars in s: [ df0e d841 ], s is valid string: false
\end{verbatim}

\subsection{Iterating over Code Points}\label{section:iterate-by-code-points}

The conventional \code{for} loop can be used to iterate over
the \code{chars} in a \code{String} or \code{CharSequence}.
%
\codeblock{FragmentsChars.14}
%
To iterate through the code points in a \code{String},
we use the method \code{codePointAt(int)}.
%
\codeblock{FragmentsChars.20}
%
This variant uses the static utility method \code{Character.charCount(int)}
to increment the index into the String's underlying \code{char} array
instead of the simple increment operator.
...
bypass the construction of a string altogether.
...
\section{Ordering Strings: Sorting and Collation}

how to sort with strings

how to sort with Collations
