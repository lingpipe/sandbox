...
\begin{figure}
\begin{center}
\includegraphics[height=2in,width=2in]{pdfs/f-measure-contour.pdf}
\end{center}
\vspace*{-24pt}
...
problem is a single first-best guess at a category.  In many cases,
particularly in LingPipe's implementations, classifiers can be
configured to return more than a single result.  These multiple
results will always be ranked from best guess to worst guess.  In
almost all cases, each result will also get a score.  In many cases,
the results are associated with conditional probability estimates of
the response category given the item being classified.
...
That's a ranked response.  Instead, they might say cabernet has a
score of -1.2, syrah a score of -1.3, and pinot a score of -2.0 (yes,
scores may be negative; they still get sorted from largest (-1.2) to
smallest (-2.0)).  That's what we call a scored response.  Even
better, the response may contain a probability estimate, such as 55\%
...
\footnote{There are also joint probability responses, from which
  conditional probabilities may be defined.  The joint probabilities
  are less useful for evaluation, so we put off considering them 
  until we see a classifier that uses them.}

\subsection{Comparing Scores across Items}

Ranks only entail that we can rank categories within the response to a
...
cabernet, and pinot, and the second one syrah, pinot and cabernet.
Without scores, there's no way to tell if the taster is more confident
about the guess of syrah for the first wine or syrah for the second
wine.
...
the document- and query-specific intricacies of scoring.

Conditional probabilities, on the other hand, are by their very nature
calibrated to be comparable across runs.  If I have two wines, and for
the first one respond 55\% cabernet, 35\% syrah and 10\% pinot, and
for the second, 90\% cabernet, 8\% syrah, and 2\% pinot, I can rank my
...
wine 1 is a cabneret (55\%), my third-best guess that wine 1 is a
...
There are a number of evaluations for systems that return scores for
each possible outcome that are calibrated across the items being
classified.  These are implemented in LingPipe's class
\code{ScoredPrecisionRecallEvaluation}, in package
\code{com.aliasi.classify}.
...
cabernet (true), 8\% syrah (false), and 2\% pinot noir (false).  We
just call \code{addCase()} with these values, as in
\code{addCase(false,0.55)}, \code{addCase(true,0.35)}, and so on down
to \code{addCase(false,0.02)}.

If the responses are exhaustive for each item, this is all we have to
do.  The key is that the correct answers will show up in every set of
responses.  Unfortunately, this is often not the case.  For instance,
consider a named-entity chunker that returns the 100-best potential
entity mentions for each sentence.  If we know the true entity
mentions, we can rank each of these results as true or false, based on
whether the entity mention is in the reference standard or not.  But
there's no guarantee that the system will find all the entities in a
sentence, even if it returns its 100-best guesses.
...
list of names or locations, because both names have lower-case
letters.  Given the input, a named-entity mention chunker might return
the following six-best ranked guesses (for instance, returning all
found chunks with estimated probability above 0.01),
%
\begin{verbatim}
...
that one of the reference entities (namely \code{(37,45):LOC}), was
not included.

The same issues of applying \code{addScore()} to all positive
responses and \code{addMisses()} for the total count of false
negatives also applies to ranked information retrieval evaluations.
...
\\ \hline
0 & $-1.21$ & no & 0 & 5 & 1 & 5 & 0.00   & 0.00   & 0.83
\\
...
\\ \hline
10 & miss & {\bf yes} & 5 & 0 & 6 & 0 & 1.00 & 0.45 & 0.00
\end{tabular}
\end{center}
...
whereas the second-best response (rank 1) has a score of $1.27$ and
is correct.  

We use this table to generate precision-recall matrices for all of the
results up to that rank.  The next four columns represent the counts
in the cells of the precision-recall matrix, TP, TN, FP, and FN, for
each rank.  These are the basis for the computation of the next three
columns, which are recall, precision, and specificity at rank $N$.
%
...
recall achieved is only 0.80 because we added a missed item, for a
total of five reference positive items.  Every time we hit a correct
answer, the recall (sensitivity) and precision go up and specificity
stays the same.
...
$\mbox{FP}=6$, for a precision of 0.4.
...
\subsubsection{Inspecting Precision-Recall and ROC Curves}
...
then printing them in code not shown).  LingPipe follows the
convention of only including points at which the system was correct,
which means values chosen from the lines in the table corresponding to
ranks 1, 3, 4, and 8.
...
curves (the code is repeated with the interpolate flag set to
\code{true}).  
%
\commandlinefollow{ant scored-precision-recall}
\begin{verbatim}
Uninterpolated P/R      Interpolated P/R
prec=0.50 rec=0.20      prec=0.60 rec=0.60
prec=0.50 rec=0.40      prec=0.44 rec=0.80
prec=0.60 rec=0.60
prec=0.44 rec=0.80
\end{verbatim}
%
We have also shown the interpolated results.  Interpolation discards a
precision-recall if there is another pair with higher precision and
higher recall.  Here the $\vecext{0.5, 0.2}$ and $\vecext{0.5, 0.4}$
operating points are removed, because the they have lower precision
and recall than the $\vecext{0.6, 0.6}$ operating point.  Of the two
remaining operating points, $\vecext{0.6,0.6}$ and $\vecext{0.44,0.8}$
are not comparable, because the first has higher precsion and the
second higher recall.  

The precision-recall curve method does not return values for the final
row(s), corresponding to positive reference items with no positive
response \ie{the misses added with \code{addMisses(int)}}.  This is
because there may be implicit false positives that were also not added
as cases.

The code for retrieving ROC curves is identical to that for
precision/recall curves, only it uses the method
\code{rocCurve(boolean)} rather than \code{prCurve(boolean)} and
returns sensitivity/specificity pairs instead of recall/precision
pairs.
%
...
%
\begin{verbatim}
Uninterpolated Spec/Sens    Interpolated Spec/Sens
spec=0.83 sens=0.20         spec=0.83 sens=0.20
spec=0.67 sens=0.40         spec=0.67 sens=0.60
spec=0.67 sens=0.60         spec=0.17 sens=0.80
spec=0.17 sens=0.80
\end{verbatim}
%
The \code{rocCurve()} method is misleadingly named, because it returns
sensitivity/specificity pairs.  The standard definition of an ROC
curve is a plot with $1 - \mbox{specificity}$ on the $x$ axis and
sensitivity on the $y$ axis.  The interpolated version removes the
...
recall (though the order doesn't matter).

\subsubsection{Other Ranked Precision-Recall Statistics}

In addition to precision-recall and ROC curves, there are a wide range
of commonly reported statistics for scored evaluations that are
implemented in LingPipe's scored precision-recall evaluation class.

Continuing with the demo, recalling that \code{eval} is our populated
instance of \code{ScoredPrecisionRecallEvaluation}, we show how to
extract other common statistics.

The scored precision-recall evaluation implements a method
\code{precisionAt(int)}, which returns the precision at a specified
rank.  This method is indexed as precision at rank is normally
indexed, starting from 1.  The precision at every rank is printed
out with the following code.
%
\codeblock{ScoredPrecisionRecallDemo.4}
%
The output corresponds to the table (adjusting for indexing from one
rather than zero).
%
\begin{verbatim}
Prec at  1=0.00     Prec at  5=0.60    Prec at  9=0.44
Prec at  2=0.50     Prec at  6=0.50    Prec at 10=0.40
Prec at  3=0.33     Prec at  7=0.43
