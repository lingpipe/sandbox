...
(prevalence,category,accuracy,item,anno,label) = pyanno.multinom.sim_dawid_skene(I,J,K)

# Calculate Sample Params
prevalence_sample = pyanno.multinom.alloc_vec(K)
for k in category:
    prevalence_sample[k] += 1
pyanno.multinom.prob_norm(prevalence_sample,Ks)
...
diff = -1
for (ll,prevalence_mle,category_mle,accuracy_mle) in pyanno.multinom.dawid_skene_mle_em(item,anno,label):
