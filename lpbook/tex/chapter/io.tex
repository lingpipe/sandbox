...
\subsection{The \code{FileLineReader} Class}
...
\subsection{The \code{Compilable} Interface}
...
\subsection{The \code{AbstractExternalizable} Class}
...
It also provides an abstract base class for implementing the
serialization proxy pattern, which was described in
\refsec{io-serialization-proxy}.  The class is declared to implement
...
implemented is \code{read(ObjectInput)}, which returns an
object.  This method is called by \code{readExternal()} and
the object it returns stored to be used as the return value
for \code{readResolve()}.
