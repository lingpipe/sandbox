...
\code{char} streams as byte streams.  In order to create a character
...
The \code{File} class operates over a system-independent
view of hierarchical pathnames.
A \techdef{pathname} consists of an optional prefix string
...
a colon-backslash (\code{:\bk}), as in \code{C:\bk} or a 
...
%
For example, \code{{\bk\bk}MONTAGUE} is the root of the shared files on the network
computer named \code{MONTAGUE}.
...
This directory is named by the system property \code{user.dir},
...
The most prominent difference between the two operating systems is
that Windows uses a backslash (\code{\bk}), whereas UNIX uses
a forward slash (\code{/}) to separate directories and file names
in a path.  The \code{File} class contains two fields: 
\code{char separatorChar} and \code{String separator} where
the String \code{File.separator} contains exactly the char
\code{File.separatorChar} for convenience.
For portable code, \code{File.separator} should be used when generating
strings that will be interpreted as pathnames during subsequent processing
...
\end{verbatim}
%
Windows platforms have a root directory for each active drive.
The root directories \code{C:\bk} and \code{D:\bk} correspond to physical
drives connected to my machine.  The root directory \code{Z:\bk} is a
mapped network drive that has been mounted.
\code{ListRoots} returns \code{Z:\bk} because it
is syntactically indistinguishable from the pathname of a local file.
File objects containing UNC pathnames are not returned by this method.
The actual network storage device \code{{\bk\bk}ls-xl5de{\bk}share}
that is mapped to drive \code{Z:\bk} is not returned by the
\code{File.listRoots()} method.

In Unix, there is a single root directory for the file system, and 
absolute path names will start with a forward slash (\code{/}).  
Running \code{list-roots} on a Mac OS X machine returns
\begin{verbatim}
Root Directories
     /
\end{verbatim}
...
%
\codeblock{FragmentsIo.3}
%
\codeblock{FragmentsIo.4}
%
...
of a file.  The method \code{exists()} tests whether it exists,
...
\subsubsection{LingPipe's \code{FileExtensionFilter}}\label{section:io-file-extension-filter}

LingPipe provides a convenience class \code{FileExtensionFilter} in
package \code{com.aliasi.io}, which implements Java's interface
\code{FileFilter} based on the extension of a file.  A file's
extension is here defined to mean the string after the final period
(\code{.}) in the file's name.  This is purely a convention for naming
files, which is especially useful in Windows or web browsers, where
each extension may be associated with a program to open or view files
with that extension.

A file extension filter takes a single string or multiple strings
specified as an array or using variable length arguments.  The filter
will match files ending with any of the extensions.
...
File extension filters work on \code{File} objects through Java's
\code{FileFilter} interface.  File extension filters do not implement
the Java interface \code{FilenameFilter} because it produces an
ambiguity when calling the methods on Java \code{File} objects
\code{listFiles(FilenameFilter)} and \code{listFiles(FileFilter}).%
%
\footnote{In general, if there are methods \code{foo(A)} and
  \code{foo(B)} in a class taking arguments specified by interfaces
  \code{A} and \code{B}, and there is an object \code{x} that
  implements both \code{A} and \code{B}, then you cannot write
  \code{foo(x)}, because the compiler doesn't know which
  implementation of \code{foo()} to use.}
%
Although it's not usually necessary, if you need to filter files based
on name, file extension filters implement a method
\code{fileNameFilter()} which returns the corresponding file name
filter implementing \code{FilenameFilter}.
...
the extension of a file, consisting of the text after the final
...
Files may be renamed using \code{renameTo(File)}, which returns \code{true}
if it succeeds.  
...
are declared to throw the more general \code{IOException}, you'll want
to catch the \code{FileNotFoundException} first.  For example, it's
legal to write
%
\codeblock{FragmentsIo.5}
%
...
Given an input stream and a character encoding, an instance of
\code{InputStreamReader} may be constructed to read characters.  The
usual idiom given that \code{encoding} is an instance of \code{String}
and \code{in} an instance of \code{InputStream} is
%
\codeblock{FragmentsIo.9}
%
...
The constructor \code{BufferedInputStream(InputStream,int)} creates a
buffered input stream wrapping the specified input stream with the
specified buffer size (in bytes).  The constructor
\code{BufferedOutputStream(OutputStream,int)} performs the same role
...
a file input stream given a \code{File} instance \code{file} with
%
\codeblock{FragmentsIo.10}
%
...
%
\footnote{We use UTF-8 encoding so frequently that LingPipe provides the
  static final string constant \code{Strings.UTF8} to use to specify
  UTF-8 encodings.}
%
\codeblock{FragmentsIo.1}
%
\codeblock{FragmentsIo.2}
...
The GZIP input and output streams are constructed by wrapping another
...
The archaically named Tape Archive (tar) format is popular, especially
...
\refsec{io-gzip}).  The Ant build library contains classes for
...
We provide a demo class \code{FileTarView} that
reads from a tar file that has optionally been gzipped.  The \code{main()}
method is defined to throw an I/O exception and read a file \code{tarFile}
and boolean flag \code{gzipped} from the first two command-line arguments.
The code begins by constructing the tar input stream.
%
\codeblock{FileTarView.1}
%
...
stream in a \code{TarInputStream}, from package
\code{org.apache.tools.tar}.

We then use a read loop, inside which we access the properties of
the entries and read the entry data.
%
\codeblock{FileTarView.2}
%
We use the \code{getNextEntry()} method of the tar input stream to get
the next instance of \code{TarEntry}, also from
\code{org.apache.tools.tar}.  If the entry is null, the stream has
...
Finally, we use LingPipe's \code{toByteArray()} utility, from class
\code{Streams} in \code{com.aliasi.util}, to read the data in the entry.
...
After we're done with the tar input stream, we close it through the
closeable interface.
%
\codeblock{FileTarView.3}
%
Tar input streams implement Java's \code{Closeable} interface, so we
close it through the \code{close()} method.  Although not documented,
closing a tar input stream will close any contained stream other than
\code{System.in}.  Here, this closes the GZIP input stream, which in
turn closes the file input stream.

\subsubsection{Running the Demo}

The Ant target \code{file-tar-view} runs the command, passing the
value of properties \code{tar.file} and \code{gzipped} as command-line
arguments.  We use the gzipped 20 newsgroups distribution as an
example (see \refsec{corpora-20-newsgroups} for a description and link).
%
\commandlinefollow{ant -Dtar.file=c:/carp/data/20news/20news-bydate.tar.gz -Dgzipped=true file-tar-view}
\begin{verbatim}
------------------------------------
...
\end{verbatim}
%
Note that we first see the directory \code{20news-bydate-test}.  The
user name is \code{jrennie}, there is no group name, and the mode
indicating permissions is \code{755} (octal).  The last-modified date
...
is \code{test/alt.atheism}, indicating that it is nested in the
previous entry.  In this corpus, it also indicates that the directory
contains postings from the \code{alt.atheism} newsgroup.  The
\code{TarEntry} object lets you access all the entries for the files
or directories it contains, but that won't let you read the data.

After the two directories, we see a file, with name
\code{bydate-test/alt.atheism/53265} and mode 644 (octal).  he
reported size matches the number of bytes read, 1994.

After the files in the \code{alt.atheism} directory, there
...
specified character encoding.  The other methods are shorthands that
...
return a \code{URLConnection}, if the URL uses the HTTP protocol, the
...
\code{InputSource(Reader)} for stream data.  Although input sources
...
\code{writeUTF(String)} method from the \code{DataOutput} interface.
...
\subsection{The \code{Serializable} Interface}\label{section:io-serializable}

The \code{Serializable} interface in package \code{java.io} is what's
known as a marker interface.  The marker pattern involves an
interface, such as \code{Serializable}, which declares no methods.
...
\code{Serializable} marker interface to determine if a class can be
serialized or not.
...
declare it to implement the \code{Serializable} interface, although it
...
implementation in \code{SerialProxied} defined here.  The class begins
by declaring it implements \code{Serializable}, then defining two
...
\code{SerialProxied} instance based on the string and count and
