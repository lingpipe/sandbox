...
This is further confused by the command-line shell.
...
For example, we can define a character class that matches both upper
and lower case instances of the letter A as: \code{[aA]}.
This syntax is confusing in that when enclosed by brackets, 
the interpretation of the sequence \code{aA} is the 
the set resulting from the union of these two atomic characters,
which will match either \code{a} or \code{A} but not \code{aA},
whereas without the brackets, the sequence \code{aA} represents
their concatenation.
...
\displ{\filepath{src/regex/src/com/lingpipe/book/regex/RegexGroup.java}}
%
The work is done in the \code{main()} method by the code
%
...
\section{String Methods that Depend on Regexes}
...
The methods \code{replaceAll(String,String)} takes
two arguments:  a string representing a regex and a replacement string
and returns the result
of replacing all the substrings in the string matching the regex
with the replacement string.
Given a regex \code{r} and replacement
...
a corresponding \code{replaceFirst()} method which is equivalent to
\code{Pattern.compile(r).matcher(this).replaceFirst(s)}.
...
Given a String regex \code{r},
the call \code{split(r)} is equivalent to calling 
\code{Pattern.compile(r).split(this,0)}.

\subsection{Performance Considerations}

The above \code{String} class utility methods all result in
calls to the \code{Regex} class which first create and compile
a new \code{Pattern} object as well as a \code{Matcher} object.
As mentioned in \refsec{pattern-compile}, sharing and reuse of 
a compiled \code{Pattern} object has the potential to
improve program efficiency.

This is most clearly seen in the case where we need to write
a program to process a text file on a line-by-line basis.
