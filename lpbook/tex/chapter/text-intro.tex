...
\firstchar{T}he canonical first program in Java (and many
...
HTML that is viewed via a web brower, \code{HelloWorld.html}
\begin{verbatim}
...
which includes \code{HelloWorld.html} so that a search for the word ``hello''
...
We favor the \stringmention{printf debugging} technique, 
whereby trace statements are used to record the flow of processing.
This technique takes its name from the \code{printf} statement in C.
In Java, we rely on the \code{System.out.println} method,
as in the \code{HelloWorld.java} example above.
...
%
\urldisplay{http://download.java.net/openjdk/jdk7/}
%
...
%
\displ{\filepath{src/intro/src/com/lingpipe/book/intro/HelloWorld.java}}
...
%
\commandlinefollow{ls}
\begin{verbatim}
build.xml	demo.properties	src
\end{verbatim}
%
We use Ant to run the demo and Ant uses the file \code{build.xml}.
An Ant build.xml file contains zero or more targets elements.
We start by invokng the \code{hello} target.
%
...
\filepath{\$BOOK/src/intro/build.xml}.  When Ant is 
...
\code{intro-4.0.jar}.  Moving onto the \code{hello} target,
...
Our second demo \code{HelloName} takes two 
...
\end{verbatim}
%
\code{HelloName} varies from \code{HelloWorld}
in the body of its \code{main()} method
%
