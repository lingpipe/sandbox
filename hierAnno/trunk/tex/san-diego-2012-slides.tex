...
\vspace*{18pt}
\noindent
\spc{\LARGE\bfseries Statistical Models}
\\[8pt]
\spc{\LARGE {\slshape of}\, {\bfseries Data Annotation}}
\\[24pt]
\spc{\large Bob Carpenter} 
\\[4pt]
\spc{\slshape Columbia University, Dept.~of Statistics}
\\[4pt]
\spc{\slshape LingPipe, Inc.}
\vfill
\noindent
\spc{\footnotesize NLP Annotation Workshop, HIISB 2012}


\sld{Why Annotate Data?}
\begin{itemize}
\item Create gold-standard corpus
  \vspace*{-4pt}
  \begin{itemize}\small
  \item estimate (train) statistical model
  \item evaluate any system
  \end{itemize}
\item Develop or validate coding standard
  \vspace*{-4pt}
  \begin{itemize}\small
  \item usually by interannotator agreement
  \end{itemize}
\end{itemize}

\sld{Traditional Approach to Annotation}
\\[8pt]
\mbox{ } \ \ {\bfseries Repeat:}
\begin{enumerate}
\item Develop or revise coding standard
\item Annotate data with two or more annotators
\item Evaluate inter-annotator agreement
\item Analyze \& adjudicate disagreements
\end{enumerate}
\vspace*{6pt}
\mbox{ } \ \ {\bfseries Until} enough data \& acceptable
agreement 

\sld{From Annotations to ``Gold'' Standard}
\begin{itemize}
\item Multiple annotators may disagree
  \vspace*{-4pt}
  \begin{itemize}\small
  \item Solicit adjudication judgment
    \begin{itemize}\small
    \item Adjudicator's judgment may not agree
    \item Changes to coding standard requires new eval data
    \end{itemize}
  \item Censor data item (more common)
  \end{itemize}
\item Don't know coder accuracy
  \vspace*{-4pt}
  \begin{itemize}\small
  \item 80\% accuracy yields 4\% error with double anno
  \item 90\% accuracy yields 1\% error
  \end{itemize}
\item Can't infer corpus accuracy without more work
\item May bias category counts to achieve ``balance''
\end{itemize}
...
\vspace*{36pt}
\noindent
\spc{\LARGE\bfseries $\kappa$ Statistics}
\vfill
(Cohen 1963, \ldots)
...
\begin{itemize}
\item Industry standard for interannotator agreement
\item Attempts to adjust for difficulty of task
\item $\kappa(A,E) = {\displaystyle\frac{A - E}{1 - E}}$
  \begin{itemize}\small
  \item $A$ is agreeement rate 
  \item $E$ is chance agreement rate
  \end{itemize}
\item Different $\kappa$ based on definition of chance agreement
\item $\kappa$ above arbitrary threshold considered acceptable
\end{itemize}

\sld{Problems with {\Huge $\kappa$}}

\begin{itemize}
\item $\kappa$ a pairwise measure over {\slshape shared}\, annotations
\item $\kappa$ useless for inference
  \vspace*{-4pt}
  \begin{itemize}\small
  \item $\kappa$ doesn't predict corpus {\slshape or}\, annotator accuracy 
  \end{itemize}
\item Two wrongs make a right
  \vspace*{-6pt}
  \begin{itemize}\small
  \item 10\% accuracy and 90\% accuracy produce same $\kappa$
  \end{itemize}
\item $\kappa$ reduces to agreement for hard problems
  ...
  \end{itemize}
\item $\kappa$ zero even with high accuracy if low (high) prevalence
  \vspace*{-6pt}
  \begin{itemize}\small
  \item 5\% positive prevalence, 95\% accuracy, then 
    $\kappa= 0$
  \end{itemize}
\end{itemize}
...
\item $\kappa$ assumes annotators are unbiased 
  ...
  \end{itemize}
\item All errors for all categories have same importance
  \vspace*{-4pt}
  \begin{itemize}\small
  \item common: low prevalence, high negative agreement
  \item common: value true positives, avoid false negatives
  \end{itemize}
\item $\kappa$ typically point estimated without uncertainty
\item $\kappa$ assumes annotations for an item are uncorrelated
  \vspace*{-4pt}
  \begin{itemize}\small
  \item if items have correlated errors, $\kappa$ too high
  \end{itemize}
\end{itemize}
...
\vspace*{36pt}
\noindent
\spc{\Large\bfseries Generative Annotation Models}
\vfill
(Dawid and Skene 1979, \ldots)

\sld{\Large Generative Story for Categorical Anno}

\noindent
\begin{itemize}
\item Estimated Quantities
  \begin{itemize}
  \item $\pi \, | \, \alpha$ \hfill distribution of categories (i.e.,
    prevalence)
\vspace*{8pt}
  \item $z_i \, | \, \pi$  \hfill true category of item $i$
\vspace*{8pt}
  \item $\theta_{j,k} \, | \, \beta_k$ \hfill response of annotator $j$ to
    category $k$
  \end{itemize}
\vspace*{8pt}
\item Observed Quantities
  \begin{itemize}
  \item $y_{i,j} \, | \, z_i, \theta_j$ \hfill label of annotator $j$ for item $i$
  \end{itemize}
\end{itemize}

\sld{Example: Dawid and Skene 1979}

\noindent
\begin{itemize}
\item Abstract: 
  {\footnotesize When compiling a patient record
    many facets are subject to errors of measurement.  A model is
    presented which allows individual error-rates to be estimated for
    polytomous facets even when the patient's ``true'' response is not
    available.  The {\slshape EM}\, algorithm is shown to provide a slow
    but sure way of obtaining maximum likelihood estimates of the
    parameters of interest.}
\item Dawid and Skene's model:
  \begin{itemize}\small
  \item $\pi \sim \mbox{Dirichlet}(1)$
  \item $z_i \sim \mbox{Categorical}(\pi)$
  \item $\theta_{j,k} \sim \mbox{Dirichlet}(1)$
  \item $y_{i,j} \sim \mbox{Categorical}(\theta_{j,z_i})$
  \end{itemize}
\end{itemize}



\sld{Annotator Response}

\begin{itemize}
\item Consider ordinal (e.g., 1--5) response (e.g., Netflix movie
  ratings; Rzhetsky, Shatkay and Wilbur's labeling of assertion
  certainty in research articles)
\item Decompose annotator response into bias and noise
  \begin{itemize}
  \item Bias toward high side (responses systematically too low)
  \item Bias toward low side (too many 1--3 responses, not enough 4--5 response)
  \item Bias toward middle (mostly 2--4 response)
  \item Bias toward extremes (mostly 1 and 5 response)
  \end{itemize}
\end{itemize}

\sld{Annotator Response {\normalsize (continued)}}
\begin{itemize}
\item Biases arise in general (misunderstanding intended scale) 
\item Biases arise at the item level (annotator missing knowledge,
  e.g., word sense)
\item Noise is non-systematic component of error
\item Can extract information from biased annotations, e.g., high-biased:
\[ \theta_j = 
   \begin{array}{r|cccc}
     & 1 & 2 & 3 & 4
     \\ \hline
...
\begin{itemize}
\item Always ask, ``What would I do if I had all the data?''
  \begin{itemize}\small
  \item If true categories ($z$) known, directly estimate prevalence ($\pi$)
  \item If true categories ($z$) and labels ($y$) known, 
    directly estimate annotator responses ($\theta_j$)
  \item If prevalence ($\pi$), annotator responses ($\theta$), and
    labels ($y$) known, estimate true category ($z$) via Bayes'
    rule
  \end{itemize}
\item Allows EM for maximum likelihood estimation
\item Allows Gibbs sampling for Bayesian posterior estimation
\end{itemize}
...
\item Need more data if the category assignments are themselves
  subject to noise (Bayesian approach sorts this all out)
\end{itemize}
\end{itemize}

\sld{Calculating Category Probabilities}
\begin{itemize}
\item Given prevalence $\pi$, annotator responses $\theta$, and labels $y$
\item Bayes's Rule 
\\[4pt]
$\begin{array}{rcl}
p(a|b) 
...
$\begin{array}{rcl}
p(c_i|y_i,\theta,\pi) 
& \propto & p(y_i|c_i,\theta,\pi) \ p(c_i|\theta,\pi) 
\\[4pt]
& = & p(y_i|c_i,\theta) \ p(c_i|\pi)
\\[4pt]
& = & p(c_i|\pi) \ \prod_{j=1}^J p(y_{i,j}|c_i,\theta)
\end{array}$
\end{itemize}
...
\sld{Calc Cat Probs: Binary Example}

\begin{itemize}
\footnotesize
\item Prevalence: \ $\pi = 0.2$
\item Specificities: \  $\theta_{1,0,0} = 0.60$; \ \ $\theta_{2,0,0} = 0.70$; \ \ $\theta_{3,0,0} = 0.80$
\item Sensitivities: \ $\theta_{1,1,1} = 0.75$; \ \ $\theta_{2,1,1} = 0.65$; \ \ $\theta_{3,1,1} = 0.90$
\item Annotations for item $i$: \ $y_{i,1} = 1$, \ $y_{i,2} = 1$, \ $y_{i,3} = 0$
\\[8pt]
\hspace*{-12pt}$\begin{array}{rcl}
\footnotesize
\mbox{\rm Pr}(c_i=1|\theta,y_i) 
& \propto & \pi \ \mbox{\rm Pr}(y_i=\langle 1,1,0 \rangle | \theta,c_i=1)
\\[4pt]
...
$\begin{array}{rcl}
\footnotesize
\mbox{\rm Pr}(c_i=0|\theta,y_i) 
& \propto & (1 - \pi) \ \mbox{\rm Pr}(y_i=\langle 1,1,0 \rangle | \theta,c_i=0)
\\[4pt]
& = & (1 - 0.2) \cdot (1 - 0.6) \cdot (1 - 0.7) \cdot 0.8 = 0.0768 
\end{array}$
\\[8pt]
\hspace*{-8pt}$\mbox{\rm Pr}(c_i=1|\theta,y_i) = \frac{\displaystyle 0.00975}{\displaystyle 0.00975+0.0768} = 0.1126516$
\end{itemize}

\sld{Supervised Inference}

\begin{itemize}
\item Simplest practical approach:
  \begin{itemize}\small
  \item Randomly sample $I$ items to label
  \item Label random subset of items with truth ($z'$)
  \item Use gold data to estimate prevalence ($\pi$)
  \item Use gold data to estimate annotator responses ($\theta_{j,k}$)
  \item Use prevalence and annotator response to estimate categories
    of remaining items $\tilde{z}$
  \end{itemize}
\end{itemize}
...
\begin{itemize}
\item Randomization necessary to avoid selection biases:
  \vspace*{-4pt}
  \begin{itemize}\small
  \item if select easy items, overestimate accuracies
  \item if select unrepresentative items, misestimate prevalence
  \item if accuracies or prevalences biased, it biases estimates of true categories
  \end{itemize}
\end{itemize}


\mypart{A Worked Example}
...
\mypart{Applications and Discussion}
...
\sld{Semi-Supervised Inference}
\begin{itemize}
\item Include some items $i$ with known categories $z_i$
\item Straightforward to include in EM for MLE or Gibbs for posterior
\item Sharpens all estimates
\end{itemize}
