...
purposes of ranking).  That's what we call a scored response.  Even
...
\code{addCase(true,0.35)}, \code{addCase(false,0.10)}, for the three
...
<p>Given the input text, a named-entity mention chunker might return
...
operating point of the same or higher recall.
...
rank.  For example, with the data in \reffig{scored-pr-example},
the precision at rank $N$ is determined by looking up the precision
for that rank.  Ranks are conventionally counted from 1, so
that is how we count for the \code{precisionAt()} method.
...
Note that this does not quite correspond to computing the convex
hull, because the results may still not be convex.  
...
The conventional approach to calculating these areas differs
between PR and ROC curves.  For PR curves, the usual approach is
to use flat interpolation.  In this case, area under the PR
curve corresponds exactly to the average of the precision values
at operating points corresponding to true positives.  These will
be evenly spaced, and thus correspond to area.
